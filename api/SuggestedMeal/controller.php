...
switch ($action) {
	/* ------------------- SuggestedMeal -------------------- */
	case 'get':
		echo Send ( "$login::SUGGESTEDMEAL::GET::ALL" );
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		$title = Get ( 'title' );
		...
		if ($uuid != '' && $title != '' && $description != '' && $rating != '' && $useCounter != '' && $shoppingUuidList != '') {
			echo Send ( "$login::SUGGESTEDMEAL::ADD::$uuid::$title::$description::$rating::$useCounter::$shoppingUuidList" );
		} else {
			echo SUGGESTED_MEAL_ERROR_PARAM;
		}
		break;
	case 'update':
		$uuid = Get ( 'uuid' );
		...
		$shoppingUuidList = Get ( 'shoppingUuidList' );
		if ($uuid != '' && $title != '' && $description != '' && $rating != '' && $useCounter != '' && $shoppingUuidList != '') {
			echo Send ( "$login::SUGGESTEDMEAL::UPDATE::$uuid::$title::$description::$rating::$useCounter::$shoppingUuidList" );
		} else {
			echo SUGGESTED_MEAL_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::SUGGESTEDMEAL::DELETE::$uuid" );
		} else {
			echo SUGGESTED_MEAL_ERROR_PARAM;
		}
		break;
