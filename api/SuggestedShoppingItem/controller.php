...
switch ($action) {
	...
	case 'get':
		echo Send ( "$login::SUGGESTEDSHOPPINGITEM::GET::ALL" );
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		$name = Get ( 'name' );
		...
		if ($uuid != '' && $name != '' && $type != '' && $quantity != '' && $unit != '') {
			echo Send ( "$login::SUGGESTEDSHOPPINGITEM::ADD::$uuid::$name::$type::$quantity::$unit" );
		} else {
			echo SUGGESTED_SHOPPING_ITEM_ERROR_PARAM;
		}
		break;
	case 'update':
		$uuid = Get ( 'uuid' );
		...
		$unit = Get ( 'unit' );
		if ($uuid != '' && $name != '' && $type != '' && $quantity != '' && $unit != '') {
			echo Send ( "$login::SUGGESTEDSHOPPINGITEM::UPDATE::$uuid::$name::$type::$quantity::$unit" );
		} else {
			echo SUGGESTED_SHOPPING_ITEM_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::SUGGESTEDSHOPPINGITEM::DELETE::$uuid" );
		} else {
			echo SUGGESTED_SHOPPING_ITEM_ERROR_PARAM;
		}
		break;
