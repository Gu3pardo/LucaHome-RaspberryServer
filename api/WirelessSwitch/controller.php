...
switch ($action) {
	/* ------------------- WirelessSwitch -------------------- */
	case 'get':
		echo Send ( "$login::WIRELESSSWITCH::GET::ALL" );
		break;
	case 'toggle':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::WIRELESSSWITCH::TOGGLE::$uuid" );
		} else {
			echo WIRELESS_SWITCH_ERROR_PARAM;
		}
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		...
		$keyCode = Get ( 'keyCode' );
		$action = Get ( 'action' );
		if ($uuid != '' && $roomUuid != '' && $name != '' && $remoteId != '' && $keyCode != '' && $action != '') {
			echo Send ( "$login::WIRELESSSWITCH::ADD::$uuid::$roomUuid::$name::$remoteId::$keyCode::$action" );
		} else {
			echo WIRELESS_SWITCH_ERROR_PARAM;
		}
		break;
	case 'update':
		...
		$action = Get ( 'action' );
		if ($uuid != '' && $roomUuid != '' && $name != '' && $remoteId != '' && $keyCode != '' && $action != '') {
			echo Send ( "$login::WIRELESSSWITCH::UPDATE::$uuid::$roomUuid::$name::$remoteId::$keyCode::$action" );
		} else {
			echo WIRELESS_SWITCH_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::WIRELESSSWITCH::DELETE::$uuid" );
		} else {
			echo WIRELESS_SWITCH_ERROR_PARAM;
		}
		break;
