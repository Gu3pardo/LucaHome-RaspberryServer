...
switch ($action) {
	...
	case 'update':
		...
		$rating = Get ( 'rating' );
		$watchCount = Get ( 'watchCount' );
		if ($uuid != '' && $title != '' && $genre != '' && $description != '' && $rating != '' && $watchCount != '') {
			echo Send ( "$login::MOVIE::UPDATE::$uuid::$title::$description::$genre::$rating::$watchCount" );
		} else {
			echo MOVIE_ERROR_PARAM;
		}
		break;
	case 'load':
