...
switch ($action) {
	...
	case 'add':
		...
		$type = Get ( 'type' );
		$coordinatesX = Get ( 'coordinatesX' );
		$coordinatesY = Get ( 'coordinatesY' );
		$name = Get ( 'name' );
		$shortName = Get ( 'shortName' );
		$visibility = Get ( 'visibility' );
		if ($uuid != '' && $typeUuid != '' && $type != '' && $coordinatesX != '' && $coordinatesY != '' && $name != '' && $shortName != '' && $visibility != '') {
			echo Send ( "$login::MAPCONTENT::ADD::$uuid::$typeUuid::$type::$coordinatesX::$coordinatesY::$name::$shortName::$visibility" );
		} else {
			echo MAP_CONTENT_ERROR_PARAM;
		}
		break;
	case 'update':
		$uuid = Get ( 'uuid' );
		...
		$visibility = Get ( 'visibility' );
		if ($uuid != '' && $typeUuid != '' && $type != '' && $coordinatesX != '' && $coordinatesY != '' && $name != '' && $shortName != '' && $visibility != '') {
			echo Send ( "$login::MAPCONTENT::UPDATE::$uuid::$typeUuid::$type::$coordinatesX::$coordinatesY::$name::$shortName::$visibility" );
		} else {
			echo MAP_CONTENT_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::MAPCONTENT::DELETE::$uuid" );
		} else {
			echo MAP_CONTENT_ERROR_PARAM;
		}
		break;
