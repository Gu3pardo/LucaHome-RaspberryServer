...
switch ($action) {
	/* ------------------- WirelessSocket -------------------- */
	case 'get':
		echo Send ( "$login::WIRELESSSOCKET::GET::ALL" );
		break;
	case 'set':
		$uuid = Get ( 'uuid' );
		$state = Get ( 'state' );
		if ($uuid != '' && $state != '') {
			echo Send ( "$login::WIRELESSSOCKET::SET::$uuid::$state" );
		} else {
			echo WIRELESS_SOCKET_ERROR_PARAM;
		}
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		...
		$name = Get ( 'name' );
		$code = Get ( 'code' );
		if ($uuid != '' && $roomUuid != '' && $name != '' && $code != '') {
			echo Send ( "$login::WIRELESSSOCKET::ADD::$uuid::$roomUuid::$name::$code" );
		} else {
			echo WIRELESS_SOCKET_ERROR_PARAM;
		}
		break;
	case 'update':
		...
		$code = Get ( 'code' );
		if ($uuid != '' && $roomUuid != '' && $name != '' && $code != '') {
			echo Send ( "$login::WIRELESSSOCKET::UPDATE::$uuid::$roomUuid::$name::$code" );
		} else {
			echo WIRELESS_SOCKET_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::WIRELESSSOCKET::DELETE::$uuid" );
		} else {
			echo WIRELESS_SOCKET_ERROR_PARAM;
		}
		break;
