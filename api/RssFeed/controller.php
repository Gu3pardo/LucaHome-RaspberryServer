...
switch ($action) {
	/* ------------------- RssFeed -------------------- */
	case 'get':
		echo Send ( "$login::RSSFEED::GET::ALL" );
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		$title = Get ( 'title' );
		...
		if ($uuid != '' && $title != '' && $url != ''&& $playCount != '') {
			echo Send ( "$login::RSSFEED::ADD::$uuid::$title::$url::$playCount" );
		} else {
			echo RSS_FEED_ERROR_PARAM;
		}
		break;
	case 'update':
		$uuid = Get ( 'uuid' );
		...
		$playCount = Get ( 'playCount' );
		if ($uuid != '' && $title != '' && $url != ''&& $playCount != '') {
			echo Send ( "$login::RSSFEED::UPDATE::$uuid::$title::$url::$playCount" );
		} else {
			echo RSS_FEED_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::RSSFEED::DELETE::$uuid" );
		} else {
			echo RSS_FEED_ERROR_PARAM;
		}
		break;
