...
switch ($action) {
	/* ------------------- User -------------------- */
	case 'validate':
		echo Send ( "$login::USER::VALIDATE::USER" );
		break;
	case 'add':
		$uuid = Get ( 'uuid' );
		$name = Get ( 'name' );
		...
		$role = Get ( 'role' );
		if ($uuid != '' && $name != '' && $passPhrase != '' && $role != '') {
			echo Send ( "$login::USER::ADD::$uuid::$name::$passPhrase::$role" );
		} else {
			echo USER_ERROR_PARAM;
		}
		break;
	case 'update':
		$uuid = Get ( 'uuid' );
		...
		$role = Get ( 'role' );
		if ($uuid != '' && $name != '' && $passPhrase != '' && $role != '') {
			echo Send ( "$login::USER::UPDATE::$uuid::$name::$passPhrase::$role" );
		} else {
			echo USER_ERROR_PARAM;
		}
		break;
	case 'delete':
		$uuid = Get ( 'uuid' );
		if ($uuid != '') {
			echo Send ( "$login::USER::DELETE::$uuid" );
		} else {
			echo USER_ERROR_PARAM;
		}
		break;
