	...
	for (int socketIndex = 0; socketIndex < socketList.size(); socketIndex++) {
		for (int index = 0; index < _socketList.size(); index++) {
			if (_socketList[index].GetName() == socketList[socketIndex]) {
				syslog(LOG_INFO, "Found socket to activate: %s", _socketList[index].GetName().c_str());
				...
				if (success) {
					syslog(LOG_INFO, "activated socket: %s", socketList[socketIndex].c_str());
				}
				else {
					syslog(LOG_INFO, "failed to activate socket: %s", socketList[socketIndex].c_str());
				}
...
void RemoteService::saveSettings(ChangeService changeService,
	...
	std::string xmldata = _xmlService.GenerateSettingsXml(_port, _datagpio,
		_receivergpio, _raspberry, _alarmSound, _wakeUpSound, _areaList,
		_sensorList, _urlList, _accessurl, _mediaMirrorList, _socketList,
		_gpioList, _scheduleList);
